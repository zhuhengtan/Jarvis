...
## macOS 长期运行与守护进程 (LaunchAgent)

Jarvis Cognitive Runtime 提供了针对 macOS 的原生系统级守护常驻方案（LaunchAgent）：
- **开机/用户登录自启**：无需打开特定终端窗口，随登录会话在后台静默运行。
- **故障自愈**：进程若发生意外崩溃，系统（launchd）将自动拉起。
- **静态托管控制台**：生产模式下由 Server 直接内置托管已构建的 Web 管理后台（`http://127.0.0.1:7330`），无 Vite 监听与热更额外内存消耗。
- **标准日志重定向**：日志规范输出至 `~/Library/Logs/Jarvis/output.log` 与 `error.log`。
...
# 5. 卸载守护进程（注销开机自启并清理 plist 文件）
pnpm daemon:uninstall
```

### 双轨端口隔离（守护常驻 vs 随时本地开发）

为了让生产常驻服务与日常本地开发**完全互不干扰**，系统设计了双轨端口隔离：

- **生产守护（常驻后台）**：
  - 端口：`7330`（HTTP API + 内置托管 Admin 控制台）与 `7331`（MCP HTTP）
  - 特性：后台静默、系统级开机自启、低内存开销。
  - Antigravity / AI IDE 默认连接此端口。
- **日常本地开发（`pnpm dev`）**：
  - 端口：自动使用 `7430`（Server API）与 `7431`（MCP HTTP），Vite 前台热更开发服务运行在 `5173` 并自动代理至 `7430`。
  - 特性：随时随地执行 `pnpm dev`，无需先停止后台守护进程，开发完毕 `Ctrl + C` 退出即可，常驻服务全程不受影响！
  - 针对开发端口测试 CLI：`pnpm cli:dev status`
