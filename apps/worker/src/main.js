...
import { FileMemoryEngine, JsonlDynamicStore, MarkdownMemoryStore, PostgresDynamicStore } from "@jarvis/memory";
...
const dynamic = process.env.DATABASE_URL ? new PostgresDynamicStore(process.env.DATABASE_URL) : new JsonlDynamicStore(resolve(memoryRoot, "..", "runtime"));
...
async function consolidate() {
    ...
    try {
        let enabled = true;
        try {
            enabled = JSON.parse(await readFile(settingsPath, "utf8")).autoConsolidation !== false;
        }
        catch (error) {
        ...
        if (result.promoted || result.archived)
            console.log(`Jarvis consolidation: promoted=${result.promoted} archived=${result.archived}`);
    }
    catch (error) {
        console.error(`Jarvis consolidation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    finally {
        running = false;
    }
