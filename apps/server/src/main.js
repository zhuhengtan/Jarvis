...
app.get("/v1/memory/:id", async (request, reply) => { const record = (await runtime.allMemories()).find((item) => item.id === request.params.id); return record ?? reply.code(404).send({ error: "Memory not found" }); });
...
app.post("/v1/memory/:id/reassign", async (request) => { const body = z.object({ projectId: z.string().min(1) }).parse(request.body); return runtime.reassignCandidate(request.params.id, body.projectId); });
